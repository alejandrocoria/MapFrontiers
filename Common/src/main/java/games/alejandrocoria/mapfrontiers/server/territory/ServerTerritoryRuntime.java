...
public class ServerTerritoryRuntime {
    ...
    public void onPlayerJoined(ServerPlayer player) {
        ...
        boolean nameChanged = playerNameRepository.observe(playerId, player.getGameProfile().getName(),
    ...
    private void hydratePlayerNamesFromMinecraftCache() {
        ...
        for (PlayerId playerId : territoriesManager.getReferencedPlayerIds()) {
            Optional<String> username = Optional.ofNullable(server.getProfileCache())
                    .flatMap(profileCache -> profileCache.get(playerId.uuid()))
