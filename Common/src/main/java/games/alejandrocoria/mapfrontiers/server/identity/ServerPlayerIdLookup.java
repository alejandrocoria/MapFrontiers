...
public final class ServerPlayerIdLookup implements PlayerIdLookup {
    ...
    private static PlayerId findOfficialProfile(MinecraftServer server, String username) {
        ...
        server.getProfileRepository().findProfilesByNames(new String[] {username}, Agent.MINECRAFT, new ProfileLookupCallback() {
            @Override
    ...
    private static boolean isValidPlayerName(String name) {
        return name.length() <= 16 && name.chars().noneMatch(character -> character <= 32 || character >= 127);
    }
}
