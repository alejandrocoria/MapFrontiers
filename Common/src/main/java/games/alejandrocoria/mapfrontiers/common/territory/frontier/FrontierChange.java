...
public class FrontierChange {
    ...
    private static GeometryChange fromGeometryEdit(GeometryEdit edit) {
        if (edit instanceof GeometryEdit.InsertPathPointAt value) {
            return new GeometryChange.InsertPathPointAt(value.index(), toBlockPos(value.point()));
        } else if (edit instanceof GeometryEdit.InsertPathPointBeforeFirst value) {
            return new GeometryChange.InsertPathPointBeforeFirst(toBlockPos(value.point()));
        } else if (edit instanceof GeometryEdit.InsertPathPointAfterLast value) {
            return new GeometryChange.InsertPathPointAfterLast(toBlockPos(value.point()));
        } else if (edit instanceof GeometryEdit.InsertPathPointAutomatically value) {
            return new GeometryChange.InsertPathPointAutomatically(toBlockPos(value.point()));
        } else if (edit instanceof GeometryEdit.SetPathPointAt value) {
            return new GeometryChange.SetPathPointAt(value.index(), toBlockPos(value.point()));
        } else if (edit instanceof GeometryEdit.RemovePathPointAt value) {
            return new GeometryChange.RemovePathPointAt(value.index());
        } else if (edit instanceof GeometryEdit.ReversePath) {
            return new GeometryChange.ReversePath();
        } else if (edit instanceof GeometryEdit.InsertVertexAt value) {
            return new GeometryChange.InsertVertexAt(value.index(), toBlockPos(value.vertex()));
        } else if (edit instanceof GeometryEdit.InsertVertexAutomatically value) {
            return new GeometryChange.InsertVertexAutomatically(toBlockPos(value.vertex()));
        } else if (edit instanceof GeometryEdit.SetVertexAt value) {
            return new GeometryChange.SetVertexAt(value.index(), toBlockPos(value.vertex()));
        } else if (edit instanceof GeometryEdit.RemoveVertexAt value) {
            return new GeometryChange.RemoveVertexAt(value.index());
        } else if (edit instanceof GeometryEdit.AddChunks value) {
            return new GeometryChange.AddChunks(value.chunks().stream()
                    .map(FrontierChange::toChunkPos)
                    .collect(java.util.stream.Collectors.toCollection(LinkedHashSet::new)));
        } else if (edit instanceof GeometryEdit.RemoveChunks value) {
            return new GeometryChange.RemoveChunks(value.chunks().stream()
                    .map(FrontierChange::toChunkPos)
                    .collect(java.util.stream.Collectors.toCollection(LinkedHashSet::new)));
        } else {
            throw new IllegalArgumentException("Unknown geometry edit: " + edit);
        }
    }
}
