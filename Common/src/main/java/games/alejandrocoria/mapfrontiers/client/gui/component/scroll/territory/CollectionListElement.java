...
public class CollectionListElement extends TerritoryListRowElement implements ScrollBox.KeyedFocusNavigation {
    ...
    private void renderTexts(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks, boolean selected) {
        ...
        graphics.drawString(font, counters, countersX, y + TITLE_Y, ColorConstants.TEXT_DIMENSION);
    ...
    private void renderMarkedCount(GuiGraphics graphics) {
        if (markedCount <= 0) {
            return;
        }
    ...
    private void drawTitle(GuiGraphics graphics, int titleX, int titleColor, String visibleTitle, boolean showExpandedTitle) {
        ...
        if (showExpandedTitle) {
            ...
            GuiGraphicsHelper.blitTinted(graphics, NAME_FADE_TEXTURE, bgOpaqueRight, y + TITLE_Y + TITLE_BG_TOP_OFFSET, 0, 0,
        ...
        graphics.drawString(font, renderedTitle, titleX, y + TITLE_Y, titleColor);
    }
    ...
    protected ScrollBox.ScrollElement.Action mousePressed(double mouseX, double mouseY, int button) {
        ...
        if (collapseToggleButton.isMouseOver(mouseX, mouseY)) {
        ...
        if (isDeleteHovered((int) mouseX, (int) mouseY)) {
            requestDelete();
            return ScrollBox.ScrollElement.Action.Handled;
        }

        if (isMoveHereHovered((int) mouseX, (int) mouseY)) {
            requestMoveHere();
            return ScrollBox.ScrollElement.Action.Handled;
        }

        if (isVisibilityHovered((int) mouseX, (int) mouseY)) {
            requestVisibility();
            return ScrollBox.ScrollElement.Action.Handled;
        }

        if (isCreateHovered((int) mouseX, (int) mouseY)) {
            requestCreate();
            return ScrollBox.ScrollElement.Action.Handled;
        }
