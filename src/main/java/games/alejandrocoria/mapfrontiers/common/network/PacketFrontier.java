            ...
            if (ctx.side == Side.CLIENT) {
                Minecraft.getMinecraft().addScheduledTask(() -> {
                    FrontierOverlay frontierOverlay = ((ClientProxy) MapFrontiers.proxy)
                            .getFrontiersOverlayManager(message.frontier.getPersonal()).addFrontier(message.frontier);
                    ...
                    if (frontierOverlay != null) {
                        if (Minecraft.getMinecraft().currentScreen instanceof GuiFrontierBook) {
                            ...
                            ((GuiShareSettings) Minecraft.getMinecraft().currentScreen).newFrontierMessage(frontierOverlay,
                                    message.playerID);
                        }
                    }
                });
